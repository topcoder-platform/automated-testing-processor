...
async function handle(message) {
  ...
  let codeRepo = _.find(_.get(challengeDetails, "metadata", {}), {
    name: "codeRepo",
  });
  ...
  if (_.intersection(tags, config.CHALLENGE_TAGS).length === 0) {
    logger.info(
      `Ignoring message as challenge with id - ${challengeId} does not contain any relevant tags. Challenge's tags: ${tags.join(
        ","
      )} whereas tags needed: ${config.CHALLENGE_TAGS}`
    );
    return;
  }
  ...
  try {
    const testFramework =
      helper.getTestFrameworkFromChallengeConfig(submissionId);

    // Create `review` with `status = queued` for the submission
    reviewObject = await reviewProducer.createReview(
      submissionId,
      null,
      "queued",
      { testType: testPhase }
    ...
    if (!helper.isUiTesting(testFramework)) {
      ...
      solutionLanguage = helper.detectSolutionLanguage(
        `${submissionPath}/submission/code/src`
      );
    ...
    if (!fs.existsSync(`${submissionPath}/submission/artifacts/private`)) {
      ...
      await fs.mkdirSync(`${submissionPath}/submission/artifacts/private`, {
        recursive: true,
      });
    }
    ...
    if (!fs.existsSync(`${submissionPath}/submission/artifacts/public`)) {
      ...
      await fs.mkdirSync(`${submissionPath}/submission/artifacts/public`, {
        recursive: true,
      });
    }
    ...
    if (!fs.existsSync(`${submissionPath}/submission/code`)) {
      logger.error(
        `Wrong folder structure detected, missing "code" folder for ${submissionId}.`
      );
      throw new Error(
        `Wrong folder structure detected, missing "code" folder for ${submissionId}.`
      );
    ...
    if (
      ...
      resultFilePath = path.join(
        `${submissionPath}/submission/artifacts/public/json-report`,
        "result.json"
      );
    } else {
      // for selenium based testing
      resultFilePath = path.join(
        `${submissionPath}/submission/artifacts/public`,
        "results.json"
      );
    }
    ...
    await reviewProducer.createReview(
      submissionId,
      score,
      "completed",
      reviewObject.metadata,
      reviewObject
    );
  } catch (error) {
